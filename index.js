...
function getArtistByIndex(arrayToReview, indexToReview) {
/*Your Code Here*/
  return `the artist at index ${indexToReview} is ${arrayToReview[indexToReview].name}`;
}  
...
function get20s(arrayToReview){
/*Your Code Here*/
  let filterArray = [];
  for (let years in arrayToReview) {
    let yearSpan = arrayToReview[years].years.split(" - ");
    if (yearSpan[0] >= 1900 && yearSpan[1] < 2000) {
      filterArray.push(arrayToReview[years].name);
    }
  }
  return filterArray;
}
...
function removeArtist(arrayToEdit, indexToRemove){
/*Your Code Here*/
  arrayToEdit.splice(indexToRemove, 1);
  return arrayToEdit.length;
}
